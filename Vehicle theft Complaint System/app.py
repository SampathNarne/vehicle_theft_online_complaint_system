...
app = Flask(__name__)
# Use environment-provided secret in production; keep a dev fallback for local runs
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'dev-key-change-in-production')

# Session cookie security - configure via environment for production deployment
# Set SESSION_COOKIE_SECURE='True' in your production environment (requires HTTPS)
...
# Database file path (use env var in production to point to a proper DB location/URL)
DB_FILE = os.environ.get('DATABASE_FILE', os.path.join(os.path.dirname(__file__), 'vehicle_theft.db'))
